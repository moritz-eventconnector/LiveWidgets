...
import BonusHuntOverlayClient from '@/components/BonusHuntOverlayClient';

export default async function BonusHuntOverlay({
  params,
  searchParams
}: {
  ...
  return (
    <div className="min-h-screen bg-transparent p-6 text-white">
      <h1 className="text-3xl font-semibold">Bonus Hunt Overlay</h1>
      <p className="mt-2 text-lg">Channel: {params.channel}</p>
      <BonusHuntOverlayClient room={`channel:${channel.id}`} />
    </div>
  );
}
